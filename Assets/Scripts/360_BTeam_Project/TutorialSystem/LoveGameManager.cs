...
public class LoveGameManager : MonoBehaviour
{
    ...
    [SerializeField] private SpriteRenderer Sprite2DMonsterImage;
    ...
    void Update()
    {
        if(player.LoveScore < -10)
        {
            ...
            Sprite2DMonsterImage.sprite = images[0];
        }
        else if(player.LoveScore >=-10 && player.LoveScore< 0)
        {
            ...
            Sprite2DMonsterImage.sprite = images[1];
        }
        else if(player.LoveScore >=0 && player.LoveScore < 10)
        {
            ...
            Sprite2DMonsterImage.sprite = images[2];
        }
        else if (player.LoveScore >= 10 && player.LoveScore < 26)
        {
            ...
            Sprite2DMonsterImage.sprite = images[3];
        }
        else if (player.LoveScore >= 26)
        {
            ...
            Sprite2DMonsterImage.sprite = images[4];
        }
        ...
        if (tController.isCompleted)
        {
            if (player.LoveScore >= 10)
            {
                Debug.Log("슬라임기쁨 결말 기쁨 결말 씬으로 이동!!!");//DEBUG
            }
            else if(player.LoveScore < -8)
            {
                Debug.Log("슬라임화남 결말 화남 결말 씬으로 이동!!!");//DEBUG
            }
            else
            {
                Debug.Log("슬라임평범 결말 평범 노말 씬으로 이동!!!");//DEBUG
            }
        }
    }
}
